...
fun aggregateDriveData(drives: List<Drive>): AggregatedDriveData {
    val totalDurationMillis = drives.sumOf { it.endTime.toEpochMilliseconds() - it.startTime.toEpochMilliseconds() }
    val averageDurationMillis = if (drives.isNotEmpty()) totalDurationMillis / drives.size else 0
